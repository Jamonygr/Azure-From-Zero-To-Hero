...
![Key Vault Secrets architecture](../assets/diagrams/security-secrets.svg)
...
| Item | Description |
|---|---|
| Main topic | Key Vault for generated admin secrets |
| Azure scope | One resource group tagged for this lesson |
| Default region | `eastus2` |
| Naming style | `clz-dev-clz340-*` |
| Cleanup path | `terraform destroy` from this folder |

## Learning Outcomes
Create vault access; write a secret; avoid exposing sensitive outputs.
...
| `key-vault.tf` | Lesson-specific Azure resources |
...
- Resource names start with `clz-dev-clz340`.
