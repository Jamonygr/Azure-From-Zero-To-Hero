...
![Cross Environment State architecture](../assets/diagrams/remote-state.svg)

## Overview
A data source that reads outputs from a shared state file.
...
| Item | Description |
|---|---|
| Main topic | Remote state data between environments |
| Azure scope | One resource group tagged for this lesson |
| Default region | `eastus2` |
| Naming style | `clz-dev-clz320-*` |
| Cleanup path | `terraform destroy` from this folder |

## Learning Outcomes
Configure terraform_remote_state; inspect shared outputs; avoid tight coupling.
...
| `remote-state-data.tf` | Lesson-specific Azure resources |
...
- Resource names start with `clz-dev-clz320`.
