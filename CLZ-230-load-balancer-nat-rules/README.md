# CLZ-230 - Load Balancer NAT Rules

![Load Balancer NAT Rules architecture](../assets/diagrams/load-balancer-vmss.svg)

## Overview
A load balancer with HTTP delivery and one scoped RDP NAT rule.
...
| Item | Description |
|---|---|
| Main topic | Controlled RDP NAT rule pattern |
| Azure scope | One resource group tagged for this lesson |
| Default region | `eastus2` |
| Naming style | `clz-dev-clz230-*` |
| Cleanup path | `terraform destroy` from this folder |

## Learning Outcomes
Understand frontend ports; associate a NAT rule; compare public and private access options.
...
| `load-balancer-nat.tf` | Lesson-specific Azure resources |
...
- Resource names start with `clz-dev-clz230`.
