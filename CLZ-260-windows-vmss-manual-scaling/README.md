# CLZ-260 - Windows VMSS Manual Scaling

![Windows VMSS Manual Scaling architecture](../assets/diagrams/load-balancer-vmss.svg)

## Overview
A Windows VMSS with IIS behind a Standard Load Balancer.
...
| Item | Description |
|---|---|
| Main topic | Windows VMSS with IIS |
| Azure scope | One resource group tagged for this lesson |
| Default region | `eastus2` |
| Naming style | `clz-dev-clz260-*` |
| Cleanup path | `terraform destroy` from this folder |

## Learning Outcomes
Create VMSS network profile; install IIS across instances; change instance_count deliberately.
...
| `vmss.tf` | Lesson-specific Azure resources |
...
- Resource names start with `clz-dev-clz260`.
