...
![Foundations architecture](../assets/diagrams/terraform-workflow.svg)

## Overview
A tagged resource group and the mental model for every later lab.
...
| Item | Description |
|---|---|
| Main topic | IaC, Terraform, and Azure lab model |
| Azure scope | One resource group tagged for this lesson |
| Default region | `eastus2` |
| Naming style | `clz-dev-clz100-*` |
| Cleanup path | `terraform destroy` from this folder |

## Learning Outcomes
Explain desired state; map Terraform files to Azure resources; read a plan before applying it.
...
- Resource names start with `clz-dev-clz100`.
...
## Next Lesson
Prepare the Windows workstation toolchain.
